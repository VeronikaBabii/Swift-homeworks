...
func routes(_ app: Application) throws {
    ...
    app.get("commands") { req -> [String] in
        return robot.getCommandsAsArrows()
    }

    // 2.1
    app.post("command") { req -> Int in
        let newCommand = try req.content.get(String.self, at: "command")
        let index = try req.content.get(Int.self, at: "index")
        return robot.addCommand(command: newCommand, at: index)
    }
